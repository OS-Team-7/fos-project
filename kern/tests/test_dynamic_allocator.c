...
bool completed_was_called = 0;
bool partial_was_called = 0;
...
void test_realloc_block_FF_COMPLETE()
{
    ...
    completed_was_called = 1;

    if(!partial_was_called)
    	test_realloc_block_FF();

	cprintf("===================================================\n");
	cprintf("*****\tMISSED TESTS FOR REALLOC_FF\t******\n") ;
	cprintf("===================================================\n");


    int initial_block_data_size, expected_block_total_size, new_size;
    void* initial_address,* expected_address,* reloc_address,* new_address;
    bool is_correct = 1;
    short eval = 0;

    //====================================================================//
    //[1] Test equal size reallocation
    //====================================================================//

    cprintf("1: test equal size reallocation.[30%]\n\n");

    initial_block_data_size = 200;

    initial_address = alloc_block_FF(initial_block_data_size);

    if(initial_address != NULL)
    {
		*((int*)initial_address) = 14052;
		*((int*)initial_address + 10) = 79563;

		new_size = initial_block_data_size;
		expected_block_total_size = initial_block_data_size + sizeOfMetaData;
		expected_address = initial_address;

		new_address = realloc_block_FF(initial_address, new_size);

		if (new_address != expected_address)
		{
			is_correct = 0;
			cprintf("**WRONG REALLOCATION - NOT THE EXPECTED ADDRESS (expected %p but got %p) !\n", expected_address, new_address);
		}
		else if(is_free_block(initial_address))
		{
			is_correct = 0;
			cprintf("**WRONG REALLOCATION - THE BLOCK BECAME FREE (IT SHOULD STILL BE ALLOCATED) !\n", expected_block_total_size, get_block_size(new_address));
		}
		else if(expected_block_total_size != get_block_size(new_address))
		{
			is_correct = 0;
			cprintf("**WRONG REALLOCATION - NOT THE EXPECTED SIZE (expected %d but got %d) !\n", expected_block_total_size, get_block_size(new_address));
		}
		else if(*((int*)initial_address) != 14052 ||
				*((int*)initial_address + 10) != 79563)
		{
			is_correct = 0;
			cprintf("**WRONG REALLOCATION - DATA HAS BEEN LOST !\n");
		}

		if(is_correct)
			eval += 30;

		free_block(new_address);
		initial_address = NULL;
    }
    else
    {
    	is_correct = 0;
    	cprintf("**FAILED TO ALLOCATE THE INITIAL BLOCK !\n");
    }
    //====================================================================//
    //[2] greater size with changing initial position
    //====================================================================//

    cprintf("2: test greater size reallocation with changing initial position.[40%]\n\n");

    //[2.1] size is too large to fit in the heap

    cprintf("\t2.1: test the case where size is too large to fit in the heap.[20%]\n\n");
    ...
    if(initial_address != NULL)
    {
		...
		*((int*)initial_address + 10) = 79563;
		...
		{
			is_correct = 0;
			cprintf("**WRONG REALLOCATION - EXPECTED NULL BUT GOT %p!\n", new_address);
		}
		else if(is_free_block(initial_address))
		{
			is_correct = 0;
			cprintf("**WRONG REALLOCATION - THE OLD BLOCK BECAME FREE THOUGH THE REALLOCATION FAILED!\n");
		}
		else if(*((int*)initial_address) != 14052 ||
				*((int*)initial_address + 10) != 79563)
		{
	...
	//[2.2] size is enough to fit in the heap

	//cprintf("\n===\nDEBUG COMPLETE REALLOC_FF TEST [2.2]\n===\n");
	//print_blocks_list(freeBlocksList);

	cprintf("\t2.2: test the case where the size can fit in the heap.[20%]\n\n");
	...
	if(initial_address != NULL)
	{
		...
		*((int*)initial_address + 10) = 79563;
		...
		expected_address = startVAs[allocCntPerSize * 6];
		expected_block_total_size = new_size + sizeOfMetaData;
		...
		else if(get_block_size(new_address) > expected_block_total_size &&
				(
						is_free_block((char*) new_address + get_block_size(new_address)) ||
						get_block_size(new_address) - expected_block_total_size >= 16
				)
			   )
		...
		{
			is_correct = 0;
			cprintf("**WRONG REALLOCATION - THE OLD BLOCK IS STILL ALLOCATED (it should be free)!\n");
		}
		else if(*((int*)new_address) != 14052 ||
				*((int*)new_address + 10) != 79563)
		{
			is_correct = 0;
			cprintf("**WRONG REALLOCATION - DATA HAS BEEN LOST AFTER REALLOCATION!\n");
		}
	...
	//====================================================================//
    //[3] Test reallocation of free block
    //====================================================================//

    cprintf("3: test reallocation of free block.[30%]\n\n");

    if(realloc_block_FF(LIST_FIRST(&freeBlocksList), 2000) != NULL)
    {
    	is_correct = 0;
    	cprintf("**WRONG REALLOCATION - REALLOCATED FREE BLOCK !\n");
    }

    if(is_correct)
    	eval += 30;

    cprintf("[COMPLETE] test realloc_block_FF completed. Evaluation = %d%\n\n", eval);
}
