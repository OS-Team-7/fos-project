...
void* sys_search_user_mem(uint32 size) {
	return search_user_mem(cur_env, size);
}
...
void sys_allocate_user_mem(uint32 virtual_address, uint32 size)
{
	...
	if (virtual_address == (uint32)NULL || virtual_address < USER_HEAP_START || virtual_address + size > USER_HEAP_MAX)
	{
	    env_exit();
	}
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_search_user_mem:
		return (uint32)sys_search_user_mem((uint32)a1);
		break;
