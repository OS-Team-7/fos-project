...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
    ...
    void* stack_start = kmalloc(KERNEL_STACK_SIZE);

    if (stack_start == NULL)
    {
        panic("create_user_kern_stack: failed to kmalloc user stack\n");
    }

    int num_of_pages = (KERNEL_STACK_SIZE / PAGE_SIZE) - 1;
    uint32* curr_page = (uint32*)stack_start;

    unsigned int physical_address = kheap_physical_address((unsigned int) *curr_page);
    struct FrameInfo* frame = to_frame_info((uint32) physical_address);

    int mapping_status = map_frame(ptr_user_page_directory, frame, *curr_page, (PERM_USER));

    if (mapping_status == E_NO_MEM)
    {
        panic("create_user_kern_stack: failed to map frames\n");
    }
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
	...
	e->START = (void*)daStart;
	e->BREAK = (void*)daStart;
	e->HARD_LIMIT = (void*)daLimit;
	e->first_free_address = (uint32)e->HARD_LIMIT + PAGE_SIZE;
