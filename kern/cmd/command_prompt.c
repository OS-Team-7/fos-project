...
int process_command(int number_of_arguments, char** arguments)
{
	...
	while (LIST_SIZE(&foundCommands) > 0) {
	        struct Command* cmd = LIST_FIRST(&foundCommands);
	...
	for(int i = 0; i < NUM_OF_COMMANDS; i++){
		...
		int argsSize = strlen(arguments[0]);
		int commSize = strlen(commands[i].name);
		int str1 = 0, str2 = 0;

		while(str1 < argsSize && str2 < commSize){
			if(commands[i].name[str2] == arguments[0][str1]){
				str1++;
			}
			str2++;
		}
