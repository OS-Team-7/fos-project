...
_main(void)
{
	...
	sys_lock_cons();
	{
		...
		//cprintf("\n Gotten vs Expected va: %u : %u \n", (uint32)z, (uint32)expectedVA);
		if (z != expectedVA) {

			panic("Get(z): Returned address is not correct. Expected = %x, Actual = %x\nMake sure that you align the allocation on 4KB boundary", expectedVA, z);
		}
		expected = 1 ; /*1table*/
	...
	sys_lock_cons();
	{
		...
		if (y != expectedVA) panic("Get(): Returned address is not correct. Expected = %u, Actual = %u, Diff = %u \n, Make sure that you align the allocation on 4KB boundary", (uint32)expectedVA, (uint32)y, (uint32)y - (uint32)expectedVA);
		expected = 0 ;
		diff = (freeFrames - sys_calculate_free_frames());
		if (diff != expected) panic("Wrong allocation (current=%d, expected=%d): make sure that you allocate the required space in the user environment and add its frames to frames_storage", freeFrames - sys_calculate_free_frames(), expected);
	}
	sys_unlock_cons();
	//sys_unlock_cons();
	//cprintf("\n y when trying to access: %u \n", (uint32)y);
	if (*y != 20) panic("Get(): Shared Variable is not created or got correctly, y: %u \n", *y) ;
